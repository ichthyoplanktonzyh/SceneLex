...
## 会话收尾（2026-08-10，P4 结束后）

- **P0-P4 全部完成并提交**（git：eb74494 → 768ca95 共 5 个提交）。
- 剩余 2 个 phase：
  - **P5**：进度界面（streak/柱状图/日程环形图，本地聚合）、设置（FSRS 设置走
    outbox 同步、账号、Danger Zone）、多语言（en + zh-Hans 起步）、本地通知
    （移动端）、词表增强（统计/搜索）。
  - **P6**：Web 打磨（快捷键）、桌面端、上云部署（Fly.io/VPS 自选，不用 CDK）、
    移动端打包。
- **接力方式已确定：新开会话**（非子智能体），首个指令为：
  `读 /Users/shadow/SceneLex/docs/v1/handoff-phase5-p6.md，然后按 P5 任务清单开始工作`。
- 接力文档已提交：`docs/v1/handoff-phase5-p6.md`（含代码地图、8 条约定、
