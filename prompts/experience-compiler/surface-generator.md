...
绑定前（pre_binding）的一切 learner-visible 字段**必须使用中文（L1）经验叙事**。
中文只能**描述可观察的经验**（动作、停顿、方向、速度、表情、距离、结果），
**不得直接给出中文等价标签**——例如不允许写"他就是不情愿的""这房间很乱"，
要写"他皱眉、慢慢站起身、步子拖得很慢""书散在桌上、抽屉开着、椅垫翻在地上"。
内部字段（semantic_spec、plan）不受此限，但绝不写进 learner-visible 文本。
...
1. **不得出现目标 L2 词**（及其屈折、派生形式）**与相邻/易混淆 L2 词**。
   （语言政策：绑定前字段一律中文。）
2. **不得出现相邻 L2 词**（同义/反义/易混淆/边界词的词形），例如目标词的反义词、
   同义词、排除条件词。用中性语言描述对比。
3. 不得出现 sense ID、内部 ref、Compiler 元数据、prompt 痕迹、字段名。
4. 不得用旁白/字幕/内心独白直接命名语义（例如“他非常不想做”），
   **也不得用中文标签直接命名**（例如“他不情愿”“房间凌乱”）。语义必须由
...
- 每个单元的交互题必须**可明确评分**：至少两个选项、恰好一个正确答案
  （is_correct），选项与反馈用**中文（L1）**描述行为证据（揭示前不得出现目标词
  与相邻词，也不得出现中文标签定义）。反馈写清“哪个行为证明了什么”。
- **正确答案的位置必须随机分布**：同一资产内约一半单元的正确答案放在第一个
  选项（a1 is_correct=true）、一半放在第二个（a2），禁止所有题目的正确项都在
  同一位置（位置不变式会让质量门失败）。不同单元的正确答案位置应交替变化。
- **正例必须真实变化变量取值**：每个单元的 episode / observable_evidence 必须
  实际呈现其 semantic_spec 中 changed_variables 的声明取值；同一资产内多个正例
  的 changed_variables 取值必须出现真实差异（例如一个正例 surface 干净、另一个
  表面有污损），禁止所有正例在某个变量上取同一值（假不变式会让质量门失败）。
- `symbol_binding.reveal` 是概念单元全部结束之后才出现的绑定点；这里**才第一次**
  出现目标 L2。`minimal_l1_gloss` 只给一个最小 L1 词/短语（如“不情愿的”），
  只作确认不展开定义；它不得被复制进任何绑定前字段。
- grounding 的 `l2_realization` 发生在揭示之后（later_post_binding），**必须**
  实际包含目标词或其自然词形，用自然 L2 语言说出；它必须能放进 source experience
  的场景（学习者刚体验过）。
- review_pool 是揭示后的复习材料（early_post_binding）：**必须使用全新经验**，
  不能重新播放首学故事；复习是反向回忆，**场景/证据/维度必须用中文（L1）描述，
  且在 reveal 之前不得出现目标词与相邻词**（学习者应先自己回忆，reveal 才显示
  L2）。
