> **已弃用。** 这是 Sense Inventory 层出现之前的 dictionary-indexed 起草提示词，
> 只在 `tools/draft.py sense <word> --num <nn> --legacy-dictionary-index` 这条
> 显式危险开关下使用，产物落在 `data/drafts/legacy-senses/`，不可 promote。
> 正常生产请用 `prompts/sense-draft.md`（由 approved Sense Inventory 驱动）。
