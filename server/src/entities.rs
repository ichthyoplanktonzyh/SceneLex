...
fn get_dt(payload: &Value, key: &str) -> Option<DateTime<Utc>> {
    get_str(payload, key).and_then(|s| {
...
/// [entity_id] is the Contract word sense id (stable string).
pub async fn upsert_learning_state(
    ...
    let word_sense_id = get_str(payload, "wordSenseId").ok_or(sqlx::Error::Protocol(
        "learning_state payload missing wordSenseId".into(),
    ))?;
    let incoming = lww_from(payload).ok_or(sqlx::Error::Protocol(
        "learning_state payload missing LWW metadata".into(),
    ))?;
    ...
    let payload_state_id = get_uuid(payload, "learningStateId").unwrap_or(Uuid::nil());
    let row_id = existing
        .as_ref()
        .map(|(_, _, _, id)| *id)
...
/// [entity_id] is the client-generated list UUID.
pub async fn upsert_list(
    ...
    let incoming = lww_from(payload).ok_or(sqlx::Error::Protocol(
        "list payload missing LWW metadata".into(),
    ))?;

    // Fork protection: an entity id must not silently migrate workspaces.
    let existing_ws: Option<Uuid> =
        sqlx::query_scalar("SELECT workspace_id FROM content.lists WHERE list_id = $1")
            .bind(entity_id)
            .fetch_optional(&mut **tx)
            .await?;
...
pub async fn upsert_workspace_settings(
    ...
    let incoming = lww_from(payload).ok_or(sqlx::Error::Protocol(
        "settings payload missing LWW metadata".into(),
    ))?;
...
pub async fn append_review_event(
    ...
    let client_event_id = get_uuid(payload, "clientEventId")
        ...
        .ok_or(sqlx::Error::Protocol(
            "review_event payload missing clientEventId".into(),
        ))?;
    let word_sense_id = get_str(payload, "wordSenseId").ok_or(sqlx::Error::Protocol(
        "review_event payload missing wordSenseId".into(),
    ))?;
    let rating = get_i64(payload, "rating").ok_or(sqlx::Error::Protocol(
        "review_event payload missing rating".into(),
    ))?;
    if !(0..=3).contains(&rating) {
        return Err(sqlx::Error::Protocol("rating out of range".into()));
    }
    let reviewed_at_client = get_dt(payload, "reviewedAtClient").ok_or(sqlx::Error::Protocol(
        "review_event payload missing reviewedAtClient".into(),
    ))?;
    let program_version = get_i64(payload, "programVersion").unwrap_or(1) as i32;
    let experience_unit_id = get_str(payload, "experienceUnitId").ok_or(sqlx::Error::Protocol(
        "review_event payload missing experienceUnitId".into(),
    ))?;
    ...
    let local_date = get_str(payload, "reviewedLocalDate")
