...
SceneLex 是一个**语义教学引擎 (Semantic Teaching Engine)**。
语言是对经验的符号化编码，词义是人类对经验范畴化后的结果。
因此，SceneLex 不使用形式化语言来描述词义，而是借助大语言模型（LLM）作为**语义编译器**，将词汇及其背后的经验分类，转化为可体验的**微世界 (Micro-world)** 呈现给学习者。

它的核心任务是实现：

```text
L2 声音 + 词汇类别 + 学习者状态 → LLM 语义编译器 → 微世界构建指令 → 渲染引擎呈现微世界
```

为每一个可教学的 **词义**（经验范畴的符号化），根据其所属的经验分类（10 种核心经验类型），采用相应的编译策略，生成能建立、区分和迁移该经验的具体场景（微世界）。

核心资产是可独立发布、版本化的语义资源。视频、交互式应用、图片、TTS 等只是微世界（呈现层）的载体，是可替换的渲染端。
下游消费者（词典、播放器、学习实验）是这些微世界与语义资源的使用者，绝不可为了某个下游消费者而扭曲核心语义模型与经验架构。

### 当前产品形态

系统采用分阶段演进策略，当前处于“经验叙事”阶段：LLM 将 WordSense + SceneSpec 编译为文本叙事，以卡片式学习单元嵌入背单词软件。每个学习单元包含：

1. 经验溯源（这个词从什么经验中长出来）
2. 5 个场景叙事（原型 / 对比 / 反例 / 边界 / 迁移）
3. L1 经验对比（母语对同一经验的不同切割）
4. 自测题

后续演进为富媒体增强（叙事 + 配图 + 音频），最终达到交互微世界（3D / 模拟 / 叙事引擎）。原型验证见 `prototype/reluctant-demo.html`。

## 核心原则

1. **经验即词义。** 语言是对经验世界的符号化编码，词汇是人类对经验进行范畴化后的符号。词义的本质是经验的范畴化+语言共同体的约定。
2. **微世界即体验。** 我们不使用苍白的形式化语言描述词义，而是让学习者在微世界中直接体验场景，通过经验实例去体会语义。
3. **词义不是词头。** 同一词头的不同含义代表不同的经验范畴，需要作为独立的教学单位。
4. **LLM 是语义编译器。** LLM 的训练权重中隐式包含了人类庞大的经验知识。我们利用 LLM 的这种隐含知识，将其作为编译器，将“词义”实例化为微世界的构建指令。
5. **区分十种经验分类并应用不同编译策略。** 不同的词汇（如实体型、动作型、心理状态型等 10 种核心类别）具有不同的经验本质，必须采用对应的独立编译策略与微世界呈现模式。
6. **三层架构解耦。** 词义（抽象层：经验的范畴化） -> 场景（语义层：经验的具体实例，即微世界指令） -> 媒体（呈现层：微世界的具体载体，如视频、交互应用）。
7. **一个词义需要证据组。** 原型、对比、反例、边界、迁移这五类场景证据（SceneSpec）作为坚实的语义层基础设施，依然有效且不可替代。
8. **多媒体无关，去视频中心化。** 视频只是微世界的可能载体之一。系统核心在语义编译，而非固定视频管线。原基于 Shot / Keyframe 的管线已降级为“视频类”微世界的实现细节。
9. **承认母语混淆与跨语言对比（L1 Confusables）。** L1 经验是学习者前置知识的一部分，在词汇的对比与边界划定时依然有效，用以澄清跨语言带来的经验映射错位。
10. **相邻概念（经验范畴）不必互斥。** 词义的边界可能是重叠、包含或不同程度的。真正不成立的情况才作为排除条件，关系逻辑显式记录在语义骨架中。
11. **生成不等于发布。** 虽然微世界由 LLM 编译生成，但质量与教学效果仍需人工 review（三层审核框架依然有效），正式资源发布需经过明确的 promote。
...
- **[Legacy 视频管线]** `docs/production-workflow.md`：原 Phase 1.4 的生产层权威说明，现降级为视频型微世界实现细节。Director 把 SceneSpec 的
  语义节拍编译成 Shot Plan；Shot Plan 是 Scene 之后的叙事执行权威，后续
...
- `prototype/`：产品原型。`reluctant-demo.html` 是经验叙事学习单元的首个原型验证。
